...
device app in turn provides functionality and controlled
access to assets to a companion client app as needed to solve
different use cases.

This document describes the threat model for the Tillitis TKey device
and the device app. Based on the system description and use cases, the
threat model tries to capture and describe the threats that needs to
be mitigated in order for the device app to work in a secure and
trustworthy manner.


## Assumptions
* There are no backdoors or vulnerabilities in Lattice iCE40 UltraPlus
  FPGA devices that allow external access to the internal configuration
  memory (NVCM) after the device configuration has been written to the
  NVCM and externak access has been locked down through the fuses.

* There is no access path to the contents of the internal, Non-Volatile
...
* UDS - Unique Device Secret. Provisioned and stored in the FPGA NVCM
  during TKey device personalization. Never to be replaced or altered
...
1. SW based. These are attacks against the TKey device that are performed
   from a client and enter the TKey device through the USB port. The SW
   attacks includes buffer flow attacks, attacks on the FW protocol.

2. HW based. These are attacks against the FPGA design of the TKey device
   as well as the PCB. The HW attacks includes faul injection, side-channel
   leakage as well as warm boot attacks. These attacks may be performed
   from the client through the USB port, through the TKey enclosure,
   or near the TKey device.
...
description tries to capture examples of possible attacks and how
the TKey device should be able to stand up against them.
...
* Prepared to invest little time (minutes) or resources - for example to
  connect a device found, try a few user supplied secrets
...
Attacks by Average Joe will come from the USB port and is SW based,
or just manual attempts. Given a hard to guess USS, the TKey Device should
withstand any attack no matter how long time the attack is allowed
...
device the FPGA configuraition is stored and locked into the NVCM. The
UDS and UDI assets are stored as part of the FPGA configuration. The
FPGA design contain some mechanisms for execution protection,
execution monitoring as well as functionality designed to make evil
maid attacks harder to successfully perform. I.e. take longer time.
...
The CH552 MCU providing USB host communication contains FW that
implements the UART communication with the FPGA. The CH552 firmware
can be updated by performing *port knocking*.  The knock sequence is
to apply 3.3V through a 10k resistor to the D+ line, while powering on
the device.

There may be possible buffer overflow attacks via the USB host
interface into the FW of the CH552, allowing both execution and
modification of the FW CH552.


#### In scope
- SW attacks from the host against the FW in the FPGA, and the FPGA
  design itself via the USB host interface.
...
Flash memory connected with SPI to the FPGA.
