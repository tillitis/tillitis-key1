...
int reset(struct reset *userreset, size_t nextlen)
{
	...
	// - seed digest = blake2s(CDI of current running app, seed digest from caller)
	...
	blake2s((void *)resetinfo->seed_digest, RESET_DIGEST_SIZE,
		(void *)cdi, 32,
		(void *)resetinfo->seed_digest, RESET_DIGEST_SIZE);
