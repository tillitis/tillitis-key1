...
uint8_t DPL; // data pointer low
uint8_t DPH; // data pointer high
uint8_t SAFE_MOD; // WriteOnly: writing safe mode
uint8_t GLOBAL_CFG; // global config, Write@SafeMode
uint8_t PCON; // power control and reset flag
uint8_t CLOCK_CFG; // system clock config: lower 3 bits for system clock Fsys, Write@SafeMode
uint8_t WAKE_CTRL; // wake-up control, Write@SafeMode
uint8_t RESET_KEEP; // value keeper during reset
uint8_t WDOG_COUNT; // watch-dog count, count by clock frequency Fsys/65536
uint8_t IE; // interrupt enable
uint8_t IP; // interrupt priority and current priority
uint8_t IE_EX; // extend interrupt enable
uint8_t IP_EX; // extend interrupt priority
uint8_t GPIO_IE; // GPIO interrupt enable
uint8_t ROM_ADDR_L; // address low byte for flash-ROM
uint8_t ROM_ADDR_H; // address high byte for flash-ROM
uint8_t ROM_DATA_L; // data low byte for flash-ROM writing, data byte for Data-Flash reading/writing
uint8_t ROM_DATA_H; // data high byte for flash-ROM writing
uint8_t ROM_CTRL; // WriteOnly: flash-ROM control
uint8_t P1; // port 1 input & output
uint8_t P1_MOD_OC; // port 1 output mode: 0=push-pull, 1=open-drain
uint8_t P1_DIR_PU; // port 1 direction for push-pull or pullup enable for open-drain
uint8_t P2; // port 2
uint8_t P3; // port 3 input & output
uint8_t P3_MOD_OC; // port 3 output mode: 0=push-pull, 1=open-drain
uint8_t P3_DIR_PU; // port 3 direction for push-pull or pullup enable for open-drain
uint8_t PIN_FUNC; // pin function selection
uint8_t XBUS_AUX; // xBUS auxiliary setting
uint8_t TCON; // timer 0/1 control and external interrupt control
uint8_t TMOD; // timer 0/1 mode
uint8_t TL0; // low byte of timer 0 count
uint8_t TL1; // low byte of timer 1 count
uint8_t TH0; // high byte of timer 0 count
uint8_t TH1; // high byte of timer 1 count
uint8_t SCON; // UART0 control (serial port control)
uint8_t SBUF; // UART0 data buffer: reading for receive, writing for transmit
uint8_t T2CON; // timer 2 control
uint8_t T2MOD; // timer 2 mode and timer 0/1/2 clock mode
uint8_t RCAP2L; // low byte of reload & capture value
uint8_t RCAP2H; // high byte of reload & capture value
uint8_t TL2; // low byte of timer 2 count
uint8_t TH2; // high byte of timer 2 count
uint8_t T2CAP1L; // ReadOnly: capture 1 value low byte for timer2
uint8_t T2CAP1H; // ReadOnly: capture 1 value high byte for timer2
uint8_t PWM_DATA2; // PWM data for PWM2
uint8_t PWM_DATA1; // PWM data for PWM1
uint8_t PWM_CTRL; // PWM 1/2 control
uint8_t PWM_CK_SE; // clock divisor setting
uint8_t SPI0_STAT; // SPI 0 status
uint8_t SPI0_DATA; // FIFO data port: reading for receive, writing for transmit
uint8_t SPI0_CTRL; // SPI 0 control
uint8_t SPI0_CK_SE; // clock divisor setting
uint8_t SPI0_SETUP; // SPI 0 setup
uint8_t SCON1;         // UART1 control (serial port control)
uint8_t SBUF1;         // UART1 data buffer: reading for receive, writing for transmit
uint8_t SBAUD1;        // UART1 baud rate setting
uint8_t ADC_CTRL; // ADC control
uint8_t ADC_CFG; // ADC config
uint8_t ADC_DATA; // ReadOnly: ADC data
uint8_t TKEY_CTRL; // touch-key control
uint8_t TKEY_DATL; // ReadOnly: low byte of touch-key data
uint8_t TKEY_DATH; // ReadOnly: high byte of touch-key data
uint8_t USB_C_CTRL; // USB type-C control
uint8_t UDEV_CTRL; // USB device physical port control
uint8_t UEP1_CTRL; // endpoint 1 control
uint8_t UEP1_T_LEN; // endpoint 1 transmit length
uint8_t UEP2_CTRL; // endpoint 2 control
uint8_t UEP2_T_LEN; // endpoint 2 transmit length
uint8_t UEP3_CTRL; // endpoint 3 control
uint8_t UEP3_T_LEN; // endpoint 3 transmit length
uint8_t USB_INT_FG; // USB interrupt flag
uint8_t USB_INT_ST; // ReadOnly: USB interrupt status
uint8_t USB_MIS_ST; // ReadOnly: USB miscellaneous status
uint8_t USB_RX_LEN; // ReadOnly: USB receive length
uint8_t UEP0_CTRL;  // endpoint 0 control
uint8_t UEP0_T_LEN; // endpoint 0 transmit length
uint8_t UEP4_CTRL;  // endpoint 4 control
uint8_t UEP4_T_LEN; // endpoint 4 transmit length
uint8_t USB_INT_EN; // USB interrupt enable
uint8_t USB_CTRL; // USB base control
uint8_t USB_DEV_AD; // USB device address, lower 7 bits for USB device address
uint8_t UEP2_DMA_L; // endpoint 2 buffer start address low byte
uint8_t UEP2_DMA_H; // endpoint 2 buffer start address high byte
uint8_t UEP3_DMA_L; // endpoint 3 buffer start address low byte
uint8_t UEP3_DMA_H; // endpoint 3 buffer start address high byte
uint8_t UEP4_1_MOD; // endpoint 4/1 mode
uint8_t UEP2_3_MOD; // endpoint 2/3 mode
uint8_t UEP0_DMA_L; // endpoint 0 buffer start address low byte
uint8_t UEP0_DMA_H; // endpoint 0 buffer start address high byte
uint8_t UEP1_DMA_L; // endpoint 1 buffer start address low byte
uint8_t UEP1_DMA_H; // endpoint 1 buffer start address high byte
uint8_t PCON; // power control and reset flag
uint8_t CLOCK_CFG; // system clock config: lower 3 bits for system clock Fsys, Write@SafeMode

// SFR16
uint16_t ROM_ADDR; // address for flash-ROM, little-endian
uint16_t ROM_DATA; // data for flash-ROM writing, little-endian
uint16_t RCAP2; // reload & capture value, little-endian
uint16_t T2COUNT; // counter, little-endian
uint16_t T2CAP1; // ReadOnly: capture 1 value for timer2
uint16_t TKEY_DAT; // ReadOnly: touch-key data, little-endian
uint16_t UEP2_DMA; // endpoint 2 buffer start address, little-endian
uint16_t UEP3_DMA; // endpoint 3 buffer start address, little-endian
uint16_t UEP0_DMA; // endpoint 0 buffer start address, little-endian
uint16_t UEP1_DMA; // endpoint 1 buffer start address, little-endian

// Bits
uint8_t CY;  // carry flag
uint8_t AC;  // auxiliary carry flag
uint8_t F0;  // bit addressable general purpose flag 0
uint8_t RS1; // register R0-R7 bank selection high bit
uint8_t RS0; // register R0-R7 bank selection low bit
uint8_t OV;  // overflow flag
uint8_t F1;  // bit addressable general purpose flag 1
uint8_t P;   // ReadOnly: parity flag
uint8_t EA;    // enable global interrupts: 0=disable, 1=enable if E_DIS=0
uint8_t E_DIS; // disable global interrupts, intend to inhibit interrupt during some flash-ROM operation: 0=enable if EA=1, 1=disable
uint8_t ET2;   // enable timer2 interrupt
uint8_t ES;    // enable UART0 interrupt
uint8_t ET1;   // enable timer1 interrupt
uint8_t EX1;   // enable external interrupt INT1
uint8_t ET0;   // enable timer0 interrupt
uint8_t EX0;   // enable external interrupt INT0
uint8_t PH_FLAG; // ReadOnly: high level priority action flag
uint8_t PL_FLAG; // ReadOnly: low level priority action flag
uint8_t PT2; // timer2 interrupt priority level
uint8_t PS;  // UART0 interrupt priority level
uint8_t PT1; // timer1 interrupt priority level
uint8_t PX1; // external interrupt INT1 priority level
uint8_t PT0; // timer0 interrupt priority level
uint8_t PX0; // external interrupt INT0 priority level
uint8_t IE_WDOG;  // enable watch-dog timer interrupt
uint8_t IE_GPIO;  // enable GPIO input interrupt
uint8_t IE_PWMX;  // enable PWM1/2 interrupt
uint8_t IE_UART1; // enable UART1 interrupt
uint8_t IE_ADC;   // enable ADC interrupt
uint8_t IE_USB;   // enable USB interrupt
uint8_t IE_TKEY;  // enable touch-key timer interrupt
uint8_t IE_SPI0;  // enable SPI0 interrupt
uint8_t SCK; // serial clock for SPI0
uint8_t TXD1; // TXD output for UART1
uint8_t TIN5; // TIN5 for Touch-Key
uint8_t MISO; // master serial data input or slave serial data output for SPI0
uint8_t RXD1; // RXD input for UART1
uint8_t TIN4; // TIN4 for Touch-Key
uint8_t MOSI; // master serial data output or slave serial data input for SPI0
uint8_t PWM1; // PWM output for PWM1
uint8_t TIN3; // TIN3 for Touch-Key
uint8_t UCC2; // CC2 for USB type-C
uint8_t AIN2; // AIN2 for ADC
uint8_t T2_; // alternate pin for T2
uint8_t CAP1_; // alternate pin for CAP1
uint8_t SCS; // slave chip-selection input for SPI0
uint8_t TIN2; // TIN2 for Touch-Key
uint8_t UCC1; // CC1 for USB type-C
uint8_t AIN1; // AIN1 for ADC
uint8_t TXD_; // alternate pin for TXD of UART0
uint8_t RXD_; // alternate pin for RXD of UART0
uint8_t T2EX; // external trigger input for timer2 reload & capture
uint8_t CAP2; // capture2 input for timer2
uint8_t TIN1; // TIN1 for Touch-Key
uint8_t VBUS2; // VBUS2 for USB type-C
uint8_t AIN0; // AIN0 for ADC
uint8_t T2; // external count input
uint8_t CAP1; // capture1 input for timer2
uint8_t TIN0; // TIN0 for Touch-Key
uint8_t UDM; // ReadOnly: pin UDM input
uint8_t UDP; // ReadOnly: pin UDP input
uint8_t T1; // external count input for timer1
uint8_t PWM2; // PWM output for PWM2
uint8_t RXD1_; // alternate pin for RXD1
uint8_t T0; // external count input for timer0
uint8_t INT1; // external interrupt 1 input
uint8_t TXD1_; // alternate pin for TXD1
uint8_t INT0; // external interrupt 0 input
uint8_t VBUS1; // VBUS1 for USB type-C
uint8_t AIN3; // AIN3 for ADC
uint8_t PWM2_; // alternate pin for PWM2
uint8_t TXD; // TXD output for UART0
uint8_t PWM1_; // alternate pin for PWM1
uint8_t RXD; // RXD input for UART0
uint8_t TF1; // timer1 overflow & interrupt flag, auto cleared when MCU enter interrupt routine
uint8_t TR1; // timer1 run enable
uint8_t TF0; // timer0 overflow & interrupt flag, auto cleared when MCU enter interrupt routine
uint8_t TR0; // timer0 run enable
uint8_t IE1; // INT1 interrupt flag, auto cleared when MCU enter interrupt routine
uint8_t IT1; // INT1 interrupt type: 0=low level action, 1=falling edge action
uint8_t IE0; // INT0 interrupt flag, auto cleared when MCU enter interrupt routine
uint8_t IT0; // INT0 interrupt type: 0=low level action, 1=falling edge action
uint8_t SM0; // UART0 mode bit0, selection data bit: 0=8 bits data, 1=9 bits data
uint8_t SM1; // UART0 mode bit1, selection baud rate: 0=fixed, 1=variable
uint8_t SM2; // enable multi-device communication in mode 2/3
uint8_t REN; // enable UART0 receive
uint8_t TB8; // the 9th transmitted data bit in mode 2/3
uint8_t RB8; // 9th data bit received in mode 2/3, or stop bit received for mode 1
uint8_t TI; // transmit interrupt flag, set by hardware after completion of a serial transmit, need software clear
uint8_t RI; // receive interrupt flag, set by hardware after completion of a serial receive, need software clear
uint8_t TF2; // timer2 overflow & interrupt flag, need software clear, the flag will not be set when either RCLK=1 or TCLK=1
uint8_t CAP1F; // timer2 capture 1 interrupt flag, set by T2 edge trigger if bT2_CAP1_EN=1, need software clear
uint8_t EXF2; // timer2 external flag, set by T2EX edge trigger if EXEN2=1, need software clear
uint8_t RCLK; // selection UART0 receive clock: 0=timer1 overflow pulse, 1=timer2 overflow pulse
uint8_t TCLK; // selection UART0 transmit clock: 0=timer1 overflow pulse, 1=timer2 overflow pulse
uint8_t EXEN2; // enable T2EX trigger function: 0=ignore T2EX, 1=trigger reload or capture by T2EX edge
uint8_t TR2; // timer2 run enable
uint8_t C_T2; // timer2 clock source selection: 0=timer base internal clock, 1=external edge counter base T2 falling edge
uint8_t CP_RL2; // timer2 function selection (force 0 if RCLK=1 or TCLK=1): 0=timer and auto reload if count overflow or T2EX edge, 1=capture by T2EX edge
uint8_t S0_FST_ACT; // ReadOnly: indicate first byte received status for SPI0
uint8_t S0_IF_OV; // interrupt flag for slave mode FIFO overflow, direct bit address clear or write 1 to clear
uint8_t S0_IF_FIRST; // interrupt flag for first byte received, direct bit address clear or write 1 to clear
uint8_t S0_IF_BYTE; // interrupt flag for a byte data exchanged, direct bit address clear or write 1 to clear or accessing FIFO to clear if bS0_AUTO_IF=1
uint8_t S0_FREE; // ReadOnly: SPI0 free status
uint8_t S0_T_FIFO; // ReadOnly: tx FIFO count for SPI0
uint8_t S0_R_FIFO; // ReadOnly: rx FIFO count for SPI0
uint8_t U1SM0;  // UART1 mode, selection data bit: 0=8 bits data, 1=9 bits data
uint8_t U1SMOD; // UART1 2X baud rate selection: 0=slow(Fsys/32/(256-SBAUD1)), 1=fast(Fsys/16/(256-SBAUD1))
uint8_t U1REN;  // enable UART1 receive
uint8_t U1TB8;  // the 9th transmitted data bit in 9 bits data mode
uint8_t U1RB8;  // 9th data bit received in 9 bits data mode, or stop bit received for 8 bits data mode
uint8_t U1TI;   // transmit interrupt flag, set by hardware after completion of a serial transmit, need software clear
uint8_t U1RI;   // receive interrupt flag, set by hardware after completion of a serial receive, need software clear
uint8_t CMPO; // ReadOnly: comparator result input
uint8_t CMP_IF; // flag for comparator result changed, direct bit address clear
uint8_t ADC_IF; // interrupt flag for ADC finished, direct bit address clear
uint8_t ADC_START; // set 1 to start ADC, auto cleared when ADC finished
uint8_t CMP_CHAN; // comparator IN- input channel selection: 0=AIN1, 1=AIN3
uint8_t ADC_CHAN1; // ADC/comparator IN+ channel selection high bit
uint8_t ADC_CHAN0; // ADC/comparator IN+ channel selection low bit
uint8_t U_IS_NAK; // ReadOnly: indicate current USB transfer is NAK received
uint8_t U_TOG_OK; // ReadOnly: indicate current USB transfer toggle is OK
uint8_t U_SIE_FREE; // ReadOnly: indicate USB SIE free status
uint8_t UIF_FIFO_OV; // FIFO overflow interrupt flag for USB, direct bit address clear or write 1 to clear
uint8_t UIF_HST_SOF; // host SOF timer interrupt flag for USB host, direct bit address clear or write 1 to clear
uint8_t UIF_SUSPEND; // USB suspend or resume event interrupt flag, direct bit address clear or write 1 to clear
uint8_t UIF_TRANSFER; // USB transfer completion interrupt flag, direct bit address clear or write 1 to clear
uint8_t UIF_DETECT; // device detected event interrupt flag for USB host mode, direct bit address clear or write 1 to clear
uint8_t UIF_BUS_RST; // bus reset event interrupt flag for USB device mode, direct bit address clear or write 1 to clear
